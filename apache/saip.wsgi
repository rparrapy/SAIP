"""Apache mod_wsgi script for saip

Point to this script in your apache config file.
A template config file was generated as the file `saip` sitting
next to this file

Make sure the apache user (normally www-data) owns your application's
deployment path (/usr/local/turbogears/saip).  On Linux machines you can
accomplish this with:

.. code-block:: bash

    sudo chown -R www-data:www-data /usr/local/turbogears/saip
...
import site
site.addsitedir('/usr/local/pythonenv/saip/lib/python2.6/site-packages')
...
import os, sys
sys.path.append('/usr/local/turbogears/saip')
...
os.environ['PYTHON_EGG_CACHE'] = '/usr/local/turbogears/saip/python-eggs'

# Initialize logging module from your TurboGears config file
from paste.script.util.logging_config import fileConfig
fileConfig('/usr/local/turbogears/saip/production.ini')

# Finally, load your application's production.ini file.
from paste.deploy import loadapp
application = loadapp('config:/usr/local/turbogears/saip/production.ini')
