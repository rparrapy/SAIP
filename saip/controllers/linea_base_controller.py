...
class LineaBaseTableFiller(TableFiller):
    ...
    def __actions__(self, obj):
        ...
        if not linea_base.cerrado:
            if TienePermiso("manage").is_met(request.environ) and cant_items > 1:
...
class LineaBaseController(CrudRestController):
    ...
    def get_all(self, *args, **kw):      
        ...
        d["permiso_unir"] = TienePermiso("manage").is_met(request.environ)
        d["model"] = "Lineas Base"
    ...
    def dividir(self, **kw):
        ...
        if "seleccionados" in kw:
            ...
            redirect('./.')
        d = dict(items = items, model = "Lineas Base", accion = 'dividir')#?pk_linea_base='+id_pŕimera_lb+'')
        return d
